...
function NoteEditForm() {
  ...
  useEffect(() => {
    // return data about note with id = id
    noteApiService.getNoteById(id).then((e) => {
      //set note data
      setNote(e.data);
    });
  }, []);
  ...
  const onSaveButtonClick = () => {
    noteApiService.updateNoteById(note._id, note).then((e) => {
      alert("Saved");
      history.push("/");
    });
  };
  ...
  return (
    <>
      <form>
        ...
        <div className="form-group">
          <button type="button" className="btn btn-primary" onClick={onSaveButtonClick}>Save changes</button>
